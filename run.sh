...
if [ "$SANDBOX_LOAD" == true ]; then
  ...
  gcloud sql operations list --instance=$SANDBOX_TARGET_INSTANCE --filter='NOT status:done' --format='value(name)' | xargs -r gcloud sql operations wait --timeout=unlimited
